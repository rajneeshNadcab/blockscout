...
document.querySelector('.txtContAddresbtn1').onclick = async function() {
 ...
 const sourceCode =  await axios.get(`https://dscscan.io/node-api/getSourceCode?address=${val1}`);
...
document.querySelector('.txtContAddresbtn2').onclick = async function() {
  ...
  const divdiff = document.querySelector('.divDiff');
 const sourceCode2 =  await axios.get(`https://dscscan.io/node-api/getSourceCode?address=${val2}`);
